...
app.get("/getEvent/:id", async (req, res) => {
  ...
  try {
    ...
    if (event) {
      res.send({ event });
      // Emit "attendeeJoined" when a user visits the event page
      io.emit("attendeeJoined", { eventId: id });
      // Increase the attendees count when a user visits the event page
      await Events.updateOne(
        { _id: id },
        { $inc: { attendees: 1 } } // Increment the attendees count
      );
    }
  } catch (error) {
    console.log(error);
